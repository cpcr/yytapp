...
function change(source) {
	...
	var sourceObj = $("form input[name='source']");
	sourceObj.val(source);
}
						...
						if (index != -1 && index != '') {
							$(data.message).each(function(i, n) {
								if (i == index) {
									...
									$("select[name='topColor'] option[value='" + n.topColor + "']").attr("selected", true);
									$("select[name='topColor'] option[value!='" + n.topColor + "']").attr("selected", false);
