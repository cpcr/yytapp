...
function scrollRefresh() {
	...
	function addList() {
		...
		$.ajax({
			type : "POST",
			url : appPath + "/easyhealth/msgcenter/list",
			...
			success : function(data) {
				...
				if (data.list.length > 0) {
					...
					$(data.list).each(function(i, n) {
						var url = appPath + "/easyhealth/msgcenter/detailView?userId=" + $('form input[name="userId"]').val() + "&id=" + n.id;
					...
					if (data.pages == data.pageNum) {
						...
						html += "<button type='button' class='btn btn-ok btn-block' onClick='changePage(" + data.nextPage + ")'>点击加载更多</button>";
						html += "</div>";
						but.append(html);
					}
