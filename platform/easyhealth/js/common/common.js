...
IS.isMacOS = /(iPhone|iPad|iPod|iOS)/.test(navigator.userAgent); //是否是IOS
IS.isAndroid = /(Android)/.test(navigator.userAgent); //是否是安卓
 ...
 * @return null
 * */
function go(url,Enabled,appCode){
	//alert(url);
    if(url){
        ...
        me.setTitle = function(){
            ...
            if(title){
                if(IS.isMacOS){
                    ...
                    try
                    {
                        yyc.appSetTitle(title);
                    } catch (e) {
                         console.log('Android的appSetTitle方法出错');
                    }
