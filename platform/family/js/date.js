...
(function($) {
	$.fn.date = function(options, Ycallback, Ncallback) {
		...
		$.fn.date.defaultOptions = {
				beginyear: nowdateY - maxAge, //日期--年--份开始
				endyear: nowdateY, //日期--年--份结束
		...
		function resetInitDete() {
			if (opts.curdate && that.val()) {
				initY = parseInt(maxAge - (nowdateY - that.val().substr(0, 4)));
				initM = parseInt(that.val().substr(5, 2));
				initD = parseInt(that.val().substr(8, 2));
			}
		}

		function initDateFirst() {
			if (initDate) {
				initY = Math.ceil(maxAge / 2);
				initM = 5;
				initD = 15;
				initDate = false;
			}
