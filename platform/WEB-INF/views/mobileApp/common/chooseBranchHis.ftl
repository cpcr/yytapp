    ...
    <div id="Hospital-list">
        ...
        <#list branchHospitals as branchHospital>
            <li onclick="goNext('${branchHospital.id}','${branchHospital.code}' , '${branchHospital.name}')">
                <div class="pic">
                    <img src="${basePath}mobileApp/images/yiyuan.png"  width="65"  height="65" />
                </div>
                <div class="info">
...
function doRefresh()
{
	...
	else if(urlType=='mobileApp/register/branchDeptes')
	{
		var freshFrom=$("<form></form>");
		freshFrom.append($('<input type="hidden" name="appId" value="${commonParams.appId}"/>'));
		freshFrom.append($('<input type="hidden" name="openId" value="${commonParams.openId}"/>'));
		...
		freshFrom.css('display','none');
		freshFrom.attr("method","post");
		freshFrom.attr("action","${basePath}mobileApp/register/index");
		freshFrom.submit();
	}
...
{	if(urlType == '/mobileApp/queue/index')
	{
		window.location.href="${basePath}mobileApp/queue/hospitalList?openId=${sessionUser.id}&appCode=easyHealth&areaCode=ShenZheng&regType=${commonParams.regType}";
	}
	else if(urlType=='mobileApp/register/branchDeptes')
	{
	...
	{
		window.location.href="${basePath}mobileApp/reports/hospitalList?openId=${sessionUser.id}&appCode=easyHealth&areaCode=ShenZheng&regType=${commonParams.regType}";
	}
	else if(urlType=='mobileApp/clinic/payIndex')
	{
		window.location.href="${basePath}mobileApp/clinic/hospitalList?openId=${sessionUser.id}&appCode=easyHealth&areaCode=ShenZheng&regType=${commonParams.regType}";
	}
	else if(urlType=='easyhealth/healthlist/reports/index')
	{
		window.location.href="${basePath}easyhealth/healthlist/reports/hospitalList?openId=${sessionUser.id}&appCode=easyHealth&areaCode=ShenZheng&regType=${commonParams.regType}";
	}
	else if(urlType=='mobileApp/medicalcard/family/toView')
	{
		window.location.href="${basePath}mobileApp/medicalcard/family/hospitalList?openId=${sessionUser.id}&appCode=easyHealth&areaCode=ShenZheng&regType=${commonParams.regType}";
	}
}
</script>