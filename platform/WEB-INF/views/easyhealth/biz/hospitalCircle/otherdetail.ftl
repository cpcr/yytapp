...
<div id="body">
    ...
    <div class="btn-w">
        <button class="btn btn-ok btn-block" onclick="invokeMap('${longitude}','${latitude}');">地图指引</button>
    </div>
    <div class="space15"></div>
</div>
...
<script>
   ...
   function invokeMap(longitude,latitude)
   {
   ...
   function toRegister()
	...
	function doGoBack()
	{
		var freshForm=$("<form></form>");
		freshForm.append($('<input type="hidden" name="locationStr" value="${locationStr}"/>'));
		freshForm.append($('<input type="hidden" name="type" value="${locateType}"/>'));
		freshForm.appendTo("body");
		freshForm.css('display','none');
		freshForm.attr("method","post");
		freshForm.attr("action","${basePath}easyhealth/hospitalCircle/toOtherIndex");
		freshForm.submit();
	}
