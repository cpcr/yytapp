...
<div id="body">
    ...
    <a href="http://jky.yunyichina.cn/mobileApp/inpatient/redirect/clinicPayRecord?appCode=alipay&appId=2015102000489434&orgAppCode=yyt48fe49dbe6eb1cee" style="display: block;padding: 5px 10px;line-height: 1.5;background: #FFFADB;color:#666;text-decoration: none;font-size: 14px;">
         温馨提示：<span style="color:#FF9100;">系统升级改造，部分医院（如深圳市第三人民医院）2月17日之后的缴费记录</span><span style="display: block;padding: 5px 10px;line-height: 1.5;background: #FFFADB;color:#666;text-decoration: underline;font-size: 14px;">请点击这里查看</span></a>
...
<form id="voForm" method="post">
    ...
    <input type="hidden" id="isInsurance" name="isInsurance" value="" />
...
<script type="text/javascript">
	...
	function loadData(){
		var url = '${basePath}mobileApp/clinic/getClinicPaidListForEasyHealth';
	...
	function formatData(data) {
		...
		$.each(data, function(i, item) {
			...
			sHtml += ' boxTable flex" data-id="' + item.mzFeeId + 
				           ...
				           '" data-cardType="' + item.cardType + 
				           '" data-patientName="' + item.patientName +
                    	   '" data-isInsurance="' + item.isInsurance +
            ...
            sHtml += '		<div class="mate">' + Number(item.totalFee / 100).toFixed(2) + '元 ';
			if(1==item.isInsurance){
				sHtml += '<span style="color:#E56600">（医保）</span>';
			}
//			else{
//                sHtml += '<span style="color:#E56600">（非医保）</span>';
//			}
			sHtml += '</div>';
	...
	function showPaidDetail(obj) {
		if (obj && ($(obj).attr('data-status') == '1' || $(obj).attr('data-status') == '30')) {
            ...
            $('#isInsurance').val(getValue($(obj).attr('data-isInsurance')));
	...
	function showNoData() {
        ...
        sHtml += '      <div class="p color666">没有缴费记录</div>';
        sHtml += '  </div>';
        sHtml += '</div>';
