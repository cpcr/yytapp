...
<form id="voForm" method="post" action="${basePath}mobileApp/register/doctorTime/index">
	...
	<input type="hidden" id="viaFlag" name="viaFlag" value="historyDoctor" />
</form>
...
<script type="text/javascript">
	...
	function loadHistoryRegDoctors() {
		...
		var url = appPath + 'easyhealth/usercenter/getHistoryDoctors';
		var now = new Date();
		...
		$.ajax({
			...
			error: function(XMLHttpRequest, textStatus, errorThrown) {
				...
				$('#voForm').attr('action', '${basePath}easyhealth/usercenter/historyRegDoctor/index?timeTemp=' + timeTemp);
				$('#voForm').submit();
			},
	...
	function formatData(data) {
		if (data.length > 0) {
			...
			$.each(data, function(i, item) {
	             ...
	             sHtml += '	<img class="topVal" src="/easyhealth/images/touxiang.png" width="60" height="60"/>';
	...
	function bindClickEvent() {
		$('.arrow').off('click').on('click', function() {
			...
			$('#selectRegDate').val('');
			console.log($('#selectRegDate').val() + '____' + $('#regType').val());
			$('#voForm').submit();
		});
	};
	...
	function showNoData() {
        ...
        sHtml += '      <div class="p color666">没有找到您的挂号医生</div>';
        sHtml += '  </div>';
        sHtml += '</div>';
