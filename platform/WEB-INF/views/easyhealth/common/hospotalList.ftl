    ...
    <div id="myCenter">
        ...
        <#list entityList as hospital>
            <li class="arrow" onclick="goNext('${hospital.appId}','${hospital.appCode}','${hospital.hospitalId}','${hospital.hospitalCode}','${hospital.hospitalName}')">
                <#-- bug:51 选医院页面 主色调为蓝色 -->
                <div class="u-img"><i class="iconfont i-blue">&#xe63e;</i></div>${hospital.hospitalName}
            </li>
        </#list>
...
function doRefresh()
{
	...
	{
		window.location.href="${basePath}easyhealth/healthlist/reports/hospitalList?openId=${sessionUser.id}&appCode=easyHealth&areaCode=ShenZheng";
	}
